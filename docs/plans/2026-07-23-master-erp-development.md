...
| 8b | RFQ, quotations, blanket POs | `@backend_agent`, `@web_agent` | 8 | **Done** — backend + `/procurement` + `/supplier` + `/procurement/blankets` |
| 9 | HR / attendance / gross payroll | `@management_app_agent`, `@backend_agent` | 2 | **Done (backend)** + Android gross-payroll desk (hours/lines/manual ded) — web staff HR already deeper; payslip PDF remains web |
...
- After `db reset`: `node supabase/seed_catalog_diagrams.mjs --docker` so PDP diagram bytes serve
