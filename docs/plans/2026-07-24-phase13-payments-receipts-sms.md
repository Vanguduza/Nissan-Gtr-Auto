...
- Edge functions (`supabase/functions/`): `contipay-initiate`, `contipay-webhook`, `process-sms-outbox` (stub OK), `process-customer-receipts` (PDF render + channel send), optional `demand-forecast`
- `supabase/tests/phase13_payments_receipts_smoke.sql`
- `packages/shared/` — payment/tender types, receipt summary builder, ContiPay status enums (no secrets)
...
| Object | Notes |
|--------|--------|
| `payment_entries` | `PE-` series; customer; currency; exchange_rate; tender (`cash`\|`bank`\|`contipay`\|`store_credit`); status draft→posted/cancelled; `posted_by`/`posted_at`; JE link |
...
| RPCs | `create_payment_entry`, `allocate_payment`, `post_payment_entry`, `cancel_payment_entry` (reversing JE), `issue_store_credit`, `redeem_store_credit`, `create_contipay_intent` (service), `mark_contipay_settled` (service), `process_receipt_outbox_batch` hooks, `generate_forecast_suggestions`, `create_mr_from_forecast` |
| Edge | ContiPay HMAC/webhook verify via env; SMS/email/WhatsApp adapters behind interface (stub → real); PDF via server lib |
...
- Full ContiPay production credential values in repo/plan (placeholders + env names only)
...
- DN-as-receipt copy; inventing ContiPay API shapes beyond initiate/webhook/settle status

## Risks / exclusions

- **Secrets:** ContiPay keys/HMAC only in Edge secrets / CI env — cite sandbox allowlist (`docs/CURSOR_BEST_PRACTICES.md`); never commit
