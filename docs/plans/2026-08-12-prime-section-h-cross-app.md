...
| H1 | **Done** | `@gtr/delivery-dispatch-worker` — Temporal host for `DeliveryDispatchWorkflow`; SQL activities via assign-bridge; Edge `delivery-dispatch-cycle` unchanged; no Fleetbase. Verifier: `pnpm --filter @gtr/delivery-dispatch-worker test` (2/2) + workflow `tsc` PASS. Live run needs `TEMPORAL_ADDRESS` + Supabase service role. **Follow-up 2026-08-14:** package.json declares `@gtr/delivery` `workspace:*` (frozen-lockfile hygiene); relative `.ts` imports kept for Node strip-types. |
