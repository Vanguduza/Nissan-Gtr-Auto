...
| H1 | **Done** | `@gtr/delivery-dispatch-worker` — Temporal host; fail-closed without `TEMPORAL_ADDRESS` + `SUPABASE_SERVICE_ROLE_KEY`; `.env.example`. **Infra ready — awaiting secrets** for live Temporal. Edge `delivery-dispatch-cycle` unchanged. Verifier: `pnpm --filter @gtr/delivery-dispatch-worker test`. |
...
| H4-del | **Done (COD dual-read + driver settlement RPC)** | Dual-read + `get_delivery_job_settlement` SECURITY DEFINER; Android delivery enriches jobs. Smoke: `delivery_job_settlement_smoke.sql`. |
