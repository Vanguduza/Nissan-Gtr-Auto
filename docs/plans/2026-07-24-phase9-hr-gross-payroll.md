...
- [ ] Employee records link to `profiles` (optional auth); HR/admin CRUD via RLS
- [ ] Attendance: clock-in/out (manual); period hours roll up for pay calc
- [ ] Gross pay = hours × rate **or** salary structure; currency `USD`|`ZIG` + rate at run time
- [ ] Manual deduction lines only; **net = gross − Σ manual**; no PAYE/NSSA/brackets/forms
- [ ] Payroll run Draft→Submit→Cancel; submitted immutable; cancel = reverse/void run (no row edit)
- [ ] Payslip export (PDF or storage object); employee/owner + HR/admin read; Storage `payslips` private
- [ ] Emit `payroll_run_ready` (and optionally `staff_no_show`) via existing outbox — no new SMS gateway
- [ ] RLS on every new table in same migration; smoke SQL asserts tax-free formula

## Paths in scope

- `supabase/migrations/20260724XXXXXX_hr_gross_payroll.sql` — schema + RPCs + RLS
...
1. Migration: `employees` + salary structure/rates + RLS (`hr`|`admin`; self-read own)
2. Attendance events + `clock_attendance` RPC (manual in/out; period hour rollup helper)
3. Payroll run/lines + manual deduction lines; `compute` / `submit` / `cancel` (gross − manual only)
4. Payslip export + Storage policy; emit `payroll_run_ready`; smoke SQL (formula + RLS denial)
...
1. Implement tasks 1–4 in `@backend_agent`
