...
| `@management_app_agent` | `vendor/coolmall-gtr/` (CoolMall UX) + `apps/android-management/` (RPC kit); behavior from `apps/web` staff; legacy = RPC discovery only | WH / HR / finance — Open POS deep-links to standalone till |
| `@pos_app_agent` | `apps/android-pos/` | Standalone adaptive counter POS (till chrome, lookup, pay, offline sync) |
