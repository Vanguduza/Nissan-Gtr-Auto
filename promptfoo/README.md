...
3. Attach eval summary (or CI artifact) to the PR.
4. A human reviewer explicitly approves “promote” of the prompt/copy; do **not** merge on green alone if asserts were weakened.
5. Offline SoR remains `file://providers/safe-narrative.js` (fixed safe narrative — avoids `echo` false positives from rule text and avoids paid `llm-rubric`).
...
| Job | When | Provider |
| --- | --- | --- |
| `promptfoo-offline` | Always (path-filtered PRs/pushes + `workflow_dispatch`) | `file://providers/safe-narrative.js` |
| `promptfoo-real` | After offline; **eval runs only if secrets present** | See below |
