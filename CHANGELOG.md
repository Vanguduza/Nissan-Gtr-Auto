...
- **H6 Done (scaffold):** OSRM `routing` profile in `docker-compose.satellites.yml`; `infra/satellites/osrm/prepare.sh` + README (Zimbabwe Geofabrik default); clients already prefer `OSRM_URL`. Runtime `osrm-routed` smoke deferred until Docker + prepared graph on host.
