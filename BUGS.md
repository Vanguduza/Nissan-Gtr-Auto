...
| --- | --- | --- | --- |
| B-MONEY-1 | Payable paths still use `NUMERIC` / JS `number` for some SQL bridges | Medium (architecture) | H4 **Done (cutover habit)** — dual-write + dual-read + shared APIs prefer `amountMinor`; Android customer cart dual-read follow-on Done; delivery dual-read still open; physical column drop + SQL arg require-minor deferred. Loyalty/credit-limit RPCs still major-only. |
