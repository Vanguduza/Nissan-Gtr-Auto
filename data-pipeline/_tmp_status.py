...
print("distinct models VISITED:", c.execute(
...
s_p = c.execute("SELECT COUNT(1) FROM queue WHERE page_type='section_list' AND status='PENDING'").fetchone()[0]
s_v = c.execute("SELECT COUNT(1) FROM queue WHERE page_type='section_list' AND status='VISITED'").fetchone()[0]
print(f"diagrams visited={d_v} pending={d_p} pct={100.0*d_v/(d_v+d_p) if d_v+d_p else 0:.1f}%")
print(f"section_list visited={s_v} pending={s_p}")
...
for r in c.execute("SELECT status, last_error, COUNT(1) FROM queue WHERE last_error IS NOT NULL AND last_error != '' GROUP BY status, last_error LIMIT 8"):
    print(" ", tuple(r))
# rough rate since 00:42 start: use max updated
row = c.execute("SELECT MIN(updated_at), MAX(updated_at) FROM queue WHERE status='VISITED' AND updated_at >= '2026-08-07 22:42:00'").fetchone()
print("visited window this run:", row)
