...
print("VISITED by model (top 12):")
for r in c.execute(
    "SELECT model_slug, COUNT(1) FROM queue WHERE status='VISITED' AND model_slug != '' GROUP BY model_slug ORDER BY 2 DESC LIMIT 12"
):
    print(" ", r)
...
print("chassis:", [r[0] for r in c.execute("SELECT DISTINCT chassis_code FROM queue WHERE chassis_code != '' ORDER BY 1")])
...
for r in c.execute("SELECT page_type, url, updated_at FROM queue WHERE status='PROCESSING'"):
    print(" ", tuple(r))
print("errors:")
for r in c.execute("SELECT last_error, COUNT(1) FROM queue WHERE last_error IS NOT NULL AND last_error != '' GROUP BY last_error LIMIT 8"):
    print(" ", tuple(r))
