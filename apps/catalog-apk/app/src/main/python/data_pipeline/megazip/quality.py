...
def bundle_quality_report(bundle: dict[str, Any]) -> dict[str, Any]:
    ...
    attrs_audit = bundle_attrs_audit(bundle)
    ...
    return {
        ...
        **diagram_stats,
        ...
        "attrs_audit": attrs_audit,
        "attrs_ok": bool(attrs_audit.get("attrs_ok")),
    }


def bundle_attrs_audit(bundle: dict[str, Any]) -> dict[str, Any]:
    """Automatic engine/year/chassis coverage report (no manual audit screen)."""
    variants = bundle.get("catalog_variants") or []
    missing_engine: list[dict[str, str]] = []
    missing_year: list[dict[str, str]] = []
    missing_chassis: list[dict[str, str]] = []
    for v in variants:
        ref = {
            "model_slug": str(v.get("model_slug") or ""),
            "variant_slug": str(v.get("slug") or ""),
            "chassis_code": str(v.get("chassis_code") or ""),
        }
        if not (v.get("chassis_code") or "").strip():
            missing_chassis.append(ref)
        if not (v.get("engine_code") or "").strip():
            missing_engine.append(ref)
        if not (v.get("year_label") or "").strip():
            missing_year.append(ref)

    fitments = bundle.get("part_fitment") or []
    fit_with_engine = sum(1 for f in fitments if (f.get("engine_code") or "").strip())
    cats = bundle.get("pnc_categories") or []
    weak_cats = [
        str(c.get("pnc_code") or "")
        for c in cats
        if not (c.get("category_name") or "").strip()
        or str(c.get("category_name") or "").lower() == "uncategorized"
    ]

    total_v = len(variants) or 1
    engine_fill = 1.0 - (len(missing_engine) / total_v) if variants else 1.0
    # Chassis is required; engine/year often absent on Megazip — attrs_ok if chassis+categories clean.
    attrs_ok = len(missing_chassis) == 0 and len(weak_cats) == 0

    return {
        "variants_total": len(variants),
        "variants_missing_engine": len(missing_engine),
        "variants_missing_year": len(missing_year),
        "variants_missing_chassis": len(missing_chassis),
        "engine_fill_ratio": round(engine_fill, 4),
        "fitments_with_engine": fit_with_engine,
        "fitments_total": len(fitments),
        "weak_category_pncs": weak_cats[:50],
        "weak_category_count": len(weak_cats),
        "missing_engine_samples": missing_engine[:25],
        "missing_year_samples": missing_year[:25],
        "missing_chassis_samples": missing_chassis[:25],
        "attrs_ok": attrs_ok,
        "note": (
            "Engine/year may be absent on Megazip variant pages; chassis + category required. "
            "HTML auto-retained when diagram/variant parse is weak."
        ),
    }
...
def assert_publishable(meta: dict[str, Any], *, strict: bool = True) -> None:
    ...
    if meta.get("attrs_audit", {}).get("variants_missing_chassis"):
        raise RuntimeError(
            "Bundle failed attrs audit: variants missing chassis_code="
            f"{meta['attrs_audit']['variants_missing_chassis']}"
        )
