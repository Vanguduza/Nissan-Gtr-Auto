...
export function StaffRfqList() {
  ...
  const refresh = useCallback(async () => {
    ...
    const rfqs = await listStaffRfqs(client);
    if (!rfqs.ok) {
  ...
  if (status.kind === "loading") {
    return <p className={styles.muted}>Loading RFQs…</p>;
  }

  if (status.kind === "auth") {
    return (
      <p className={styles.lede}>
        <Link href="/login">Sign in</Link> with a staff account (admin,
        warehouse, or finance) to manage RFQs.
      </p>
    );
  }
    ...
    <ul className={styles.list}>
      {status.rfqs.map((rfq) => (
        <li key={rfq.id}>
          ...
          <br />
          <Link href={`/procurement/rfqs/${rfq.id}`} className={styles.btn}>
            Open
          </Link>
        </li>
      ))}
