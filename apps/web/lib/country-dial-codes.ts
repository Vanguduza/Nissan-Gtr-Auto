/** ITU-T E.164 country calling codes for phone pickers. */
export type CountryDialCode = {
  /** ISO 3166-1 alpha-2 */
  iso: string;
  name: string;
  /** Including leading +, e.g. "+263" */
  dial: string;
};

export const DEFAULT_COUNTRY_DIAL = "+263";

/**
 * Common + all ITU assigned country codes (sorted by name).
 * Duplicate dial codes (e.g. +1 NANP) appear once per territory where helpful.
 */
...
/** Option value uniquely identifies a dial row (dial may repeat across territories). */
export function countryDialOptionValue(c: CountryDialCode): string {
  return `${c.iso}:${c.dial}`;
}

export function parseCountryDialOption(
  value: string,
): { iso: string; dial: string } | null {
  const i = value.indexOf(":");
  if (i <= 0) return null;
  return { iso: value.slice(0, i), dial: value.slice(i + 1) };
}

/** Digits only; strip a single leading trunk 0 common in local numbers. */
export function nationalDigitsOnly(raw: string): string {
...
export function toE164(dial: string, national: string): string {
  ...
  return `${code}${nationalDigitsOnly(national)}`;
}
