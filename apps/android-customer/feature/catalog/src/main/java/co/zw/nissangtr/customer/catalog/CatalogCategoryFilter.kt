 ...
 */
internal object CatalogCategoryFilter {
    ...
    /** PostgREST `or=(category_name.ilike.*n*,subcategory_name.ilike.*n*,…)` */
    fun postgrestOrFilter(filter: String): String? {
        val ns = needles(filter).take(8)
        if (ns.isEmpty()) return null
        return ns.flatMap { n ->
            val pat = "%${escapeIlike(n)}%"
            listOf("category_name.ilike.$pat", "subcategory_name.ilike.$pat")
        }.joinToString(",")
    }
    ...
    private fun stem(raw: String): String? {
        ...
        return stemmed.takeIf { it.length >= 3 && it != t }
            ?: stemmed.takeIf { it.length >= 3 }
    }
