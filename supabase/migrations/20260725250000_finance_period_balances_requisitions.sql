...
AS $$
  SELECT COALESCE(
    nullif(trim(both '"' FROM s.value_json::text), ''),
    '1100'
  )::varchar(10)
  FROM public.app_settings s
  WHERE s.key = 'finance.petty_cash_funding_account_code'
  UNION ALL
  SELECT '1100'::varchar(10)
  WHERE NOT EXISTS (
    SELECT 1 FROM public.app_settings
    WHERE key = 'finance.petty_cash_funding_account_code'
  )
  LIMIT 1;
$$;
...
BEGIN
  ...
  SELECT e.posted_at, e.entry_date
  INTO v_since, v_since_date
  ...
  WHERE e.status = 'posted'
    ...
    AND (
      v_since IS NULL
      OR e.entry_date > v_since_date
      OR (e.entry_date = v_since_date AND e.posted_at > v_since)
      OR (e.entry_date = v_since_date AND e.posted_at IS NOT DISTINCT FROM v_since AND e.id > (
        SELECT e2.id FROM public.journal_entries e2
        WHERE e2.posted_at IS NOT DISTINCT FROM v_since
          AND e2.entry_date = v_since_date
        ORDER BY e2.id DESC
        LIMIT 1
      ))
    );

  -- Simpler post-replenish filter when last float found: entries strictly after that JE
  IF v_since IS NOT NULL THEN
    ...
    WHERE e.status = 'posted'
      ...
      AND l.credit > 0
      AND e.entry_date <= COALESCE(p_as_of, CURRENT_DATE)
      AND (e.entry_date, e.posted_at, e.id) > (v_since_date, v_since, (
        SELECT x.id
        FROM public.journal_entries x
        WHERE x.status = 'posted'
          AND x.currency = p_currency
          AND x.entry_date = v_since_date
          AND x.posted_at IS NOT DISTINCT FROM v_since
          AND EXISTS (
            SELECT 1 FROM public.journal_entry_lines d
            WHERE d.journal_entry_id = x.id AND d.account_code = '1110' AND d.debit > 0
          )
          AND EXISTS (
            SELECT 1 FROM public.journal_entry_lines c
            WHERE c.journal_entry_id = x.id AND c.account_code = v_fund AND c.credit > 0
          )
        ORDER BY x.id DESC
        LIMIT 1
      ));
