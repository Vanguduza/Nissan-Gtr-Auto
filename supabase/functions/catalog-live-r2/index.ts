...
const REQUIRED_SERVING_KINDS = ["vehicle_search", "vehicle_fitment", "section_parts", "diagram_parts", "diagram_image"] as const;
...
Deno.serve(async (req) => {
  ...
  if (action === "staff-families") {
    ...
    const { data, error } = await admin.from("catalog_models").select("id,slug,display_name,body_type,year_start,year_end").eq("maker_slug", maker).order("sort_key");
    if (error) return reply(req, 503, { error: "catalog hierarchy unavailable" });
    const families = (data ?? []).map((row) => ({ family_id: row.id, family_slug: row.slug, display_name: row.display_name, generation_label: row.year_start || row.year_end ? `${row.year_start ?? ""}${row.year_end ? `–${row.year_end}` : ""}` : null, body_type: row.body_type ?? null, year_start: row.year_start ?? null, year_end: row.year_end ?? null }));
    return reply(req, 200, { release: release.version, families, full_catalog_download_required: false });
  }
  if (action === "staff-variants") {
    if (!(await isStaff())) return reply(req, 403, { error: "staff access required" });
    const familyId = (param("family_id") ?? "").trim();
    if (!familyId) return reply(req, 400, { error: "family_id required" });
    const { data: family } = await admin.from("catalog_models").select("slug").eq("id", familyId).eq("maker_slug", maker).maybeSingle();
    if (!family?.slug) return reply(req, 404, { error: "catalog family not found" });
    const { data, error } = await admin.from("catalog_variants").select("id,slug,chassis_code,frame,grade,sales_region,year_label,engine_code").eq("maker_slug", maker).eq("model_slug", family.slug).order("chassis_code");
    if (error) return reply(req, 503, { error: "catalog hierarchy unavailable" });
    const variants = (data ?? []).map((row) => ({ variant_id: row.id, variant_slug: row.slug, display_name: [row.grade, row.chassis_code, row.engine_code, row.year_label].filter(Boolean).join(" · ") || row.slug, body_style: row.frame ?? null, drive_type: null, transmission_code: null, market: row.sales_region ?? null }));
  ...
  if (action === "staff-sections") {
    ...
    const { data: variant } = await admin.from("catalog_variants").select("model_slug,slug").eq("id", variantId).eq("maker_slug", maker).maybeSingle();
    if (!variant?.slug) return reply(req, 404, { error: "catalog variant not found" });
    const [{ data: rows, error }, { data: diagramRows }] = await Promise.all([
      admin.from("catalog_sections").select("id,slug,name,sort_order").eq("maker_slug", maker).eq("model_slug", variant.model_slug).eq("variant_slug", variant.slug).order("sort_order"),
      admin.from("catalog_diagrams").select("section_slug").eq("maker_slug", maker).eq("model_slug", variant.model_slug).eq("variant_slug", variant.slug),
    ]);
    if (error) return reply(req, 503, { error: "catalog hierarchy unavailable" });
    const counts = new Map<string, number>();
    for (const row of diagramRows ?? []) counts.set(row.section_slug, (counts.get(row.section_slug) ?? 0) + 1);
    const sections = (rows ?? []).map((row) => ({ section_id: row.id, section_slug: row.slug, display_name: row.name, sort_order: row.sort_order ?? 0, diagram_count: counts.get(row.slug) ?? 0 }));
    return reply(req, 200, { release: release.version, sections, full_catalog_download_required: false });
  }
  if (action === "staff-diagrams") {
    if (!(await isStaff())) return reply(req, 403, { error: "staff access required" });
    const familySlug = (param("family_slug") ?? "").trim();
    const variantSlug = (param("variant_slug") ?? "").trim();
    const sectionSlug = (param("section_slug") ?? "").trim();
    if (!familySlug || !variantSlug || !sectionSlug) return reply(req, 400, { error: "family_slug, variant_slug and section_slug required" });
    const limit = Math.max(1, Math.min(Number(param("limit") ?? "200") || 200, 500));
    const offset = Math.max(0, Number(param("offset") ?? "0") || 0);
    const { data: rows, error } = await admin.from("catalog_diagrams").select("id,slug,title,diagram_kind,hotspot_count").eq("maker_slug", maker).eq("model_slug", familySlug).eq("variant_slug", variantSlug).eq("section_slug", sectionSlug).order("slug").range(offset, offset + limit - 1);
    if (error) return reply(req, 503, { error: "catalog hierarchy unavailable" });
    ...
    if (ids.length) {
      ...
      for (const row of imageRows ?? []) ready.add(row.scope_key);
    }
    const diagrams = (rows ?? []).map((row) => ({ diagram_id: row.id, title: row.title ?? null, name_en: row.title ?? null, diagram_kind: row.diagram_kind ?? null, applicability: null, expected_part_count: null, expected_hotspot_count: row.hotspot_count ?? null, image_ready: ready.has(row.id), image_sha256: null }));
    return reply(req, 200, { release: release.version, diagrams, full_catalog_download_required: false });
  }
  ...
  if (action === "health") {
    ...
    return reply(req, 200, { release: release.version, object_counts: counts, r2_serving_objects: Object.values(counts).reduce((sum, value) => sum + Number(value ?? 0), 0), missing_required_kinds: missing, r2_configured: Boolean(r2), live_browsing_ready: Boolean(r2) && missing.length === 0, full_catalog_download_required: false });
  }
  ...
  return reply(req, 404, { error: "unknown action", actions: ["health", "customer-stock", "customer-search", "fitment-check", "staff-families", "staff-variants", "staff-sections", "staff-diagrams", "staff-section-parts", "staff-diagram-parts", "diagram-image"] });
});
